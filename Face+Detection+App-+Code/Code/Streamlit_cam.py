...
st.markdown(
    "<html><I>Make sure that you have Python 3.6 - Python 3.8 installed.</I><br></html>",
...
How_is_streamlit = st.selectbox(
    "likings: ", ["Very much", "So so", "Boring", "Useless"]
)
...
components.html(
    "<html><body>Note: The haar cascade files can be downloaded from the "
    '<a href = "https://github.com/opencv/opencv/tree/master/data/haarcascades">'
    "OpenCV Github repository</a></body></html>"
)

st.write(
    "For Example, if you go to the github page of haarcascade you will see that there is a particular"
...
st.write(
    "We will use the cv::CascadeClassifier class to detect objects in a video stream. Particularly,"
    " we will use the functions:"
)
st.write(
    "-> cv::CascadeClassifier::load to load a .xml classifier file. It can be either a Haar or a LBP "
    "classifier"
)
...
st.subheader("Step 1:")
st.write(
    "We use the CascadeClassifier function of OpenCV to point to the location where we have "
...
st.info(r'cascPath = "C:\Users\User\Desktop\Data science projects\Face+Detection+App-+Code\Code\haarcascade_frontalface_default.xml"')
st.info("faceCascade = cv2.CascadeClassifier(cascPath)")

st.subheader("Step 2:")
st.write(
    "Now the 2nd step is to load the frames and convert it into gray-scale."
...
st.subheader("Step 4:")
st.write(
    "From the above step, the function detectMultiScale returns 4 values — x-coordinate, "
...
start = col1.button("Start Face Detection")
...
if start:
    ...
    while True:
        ...
        if not video_capture.isOpened():
            st.error("Unable to load camera. Please check your webcam connection.")
            sleep(5)
            continue
        ...
        if frame_count % 30 == 0:  # Update FPS every 30 frames
            ...
            fps = frame_count / (current_time - start_time).total_seconds()
